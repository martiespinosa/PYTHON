...
global palabraElejida, paraulaUsuari


def validar_y_mover(event, entry_actual, entry_siguiente):
    """ Funcion que valida que el caracter introducido es una letra y mueve el foco firectamente al siguiente entry.
        Para una escritura fluida de la palabra que consta de 5 entrys diferentes (uno para cada letra)
...
def lasQueNoEstan(entry_actual):
    """ Funcion para poner el borde de los entry en rojo de las letras que el usuario ya ha descubierto que no estan en la palabra.
        Para facilitar el juego al usuario
    """
...
def borrar_y_mover(event, entry_actual, entry_anterior):
    """ Funcion para borrar el último entry y mover el foco al entry anterior.

    """
    if entry_actual != labelWordle and event.keysym == "BackSpace":
...
def check_fullscreen(root):
    """ Funcion que comprueba si la app esta en fullscreen o no
    """
...
def update_ui(root):
    """ Funcion que adapta la medida de todos los widgets según si la app esta en fullscreen o no
...
def enter(event, entry_actual, entry_siguiente, entry_f1_c1, entry_f1_c2, entry_f1_c3, entry_f1_c4, entry_f1_c5, entry_f2_c1, entry_f2_c2, entry_f2_c3, entry_f2_c4, entry_f2_c5, entry_f3_c1, entry_f3_c2, entry_f3_c3, entry_f3_c4, entry_f3_c5, entry_f4_c1, entry_f4_c2, entry_f4_c3, entry_f4_c4, entry_f4_c5, entry_f5_c1, entry_f5_c2, entry_f5_c3, entry_f5_c4, entry_f5_c5, entry_f6_c1, entry_f6_c2, entry_f6_c3, entry_f6_c4, entry_f6_c5):
    """ Funcion que colorea cada entry de un color según si esa letra está en la palabra y está en su sitio
    """
                ...
                if paraulaUsuari == todasLasPalabrasDeUnaEnUna:
                    if paraulaUsuari != palabraElejida:
                        ...
                        if idiomaActual == "Español":
                            bbdd = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PALABRAS_WORDLE_1K")
                        elif idiomaActual == "Català":
                            bbdd = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PARAULES_WORDLE_1K")
                        elif idiomaActual == "English":
                            bbdd = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\WORDS_WORDLE_1K")    

                        miConexion = bbdd
                    
                        miCursor = miConexion.cursor()
                        miCursor.execute("SELECT ULTIMA_FECHA FROM ESTADISTICAS WHERE id = 1")
                        fechaBBDD = miCursor.fetchone()[0]
                        
                        if str(fechaBBDD) != str(fecha_actual):
                            miCursor.execute("UPDATE ESTADISTICAS SET DEFEATS = DEFEATS + 1")
                            miCursor.execute("UPDATE ESTADISTICAS SET ULTIMA_FECHA = ? WHERE id = 1", (fecha_actual,))       
                        miConexion.commit()
                        miConexion.close()
        ...
        for i in range(int(numeroPalabrasTotal)):
            ...
            if paraulaUsuari == todasLasPalabrasDeUnaEnUna:
                ...
                if paraulaUsuari == palabraElejida:                                  
                    ...
                    if str(fechaBBDD) != str(fecha_actual):
                        miCursor.execute("UPDATE ESTADISTICAS SET WINS = WINS + 1")
                        miCursor.execute("UPDATE ESTADISTICAS SET ULTIMA_FECHA = ? WHERE id = 1", (fecha_actual,))   

                        if entry_f2_c1.get() == "":
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA1 = FILA1 + 1")
                        elif entry_f3_c1.get() == "":
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA2 = FILA2 + 1")
                        elif entry_f4_c1.get() == "":
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA3 = FILA3 + 1")
                        elif entry_f5_c1.get() == "":
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA4 = FILA4 + 1")
                        elif entry_f6_c1.get() == "":
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA5 = FILA5 + 1")
                        else:
                            miCursor.execute("UPDATE ESTADISTICAS SET FILA6 = FILA6 + 1")

                    miConexion.commit()
                    miConexion.close()
                    
                    app.after(1000, gameWin)
...
# def bbdds():
bbdd1k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PALABRAS_WORDLE_1K")
bbdd10k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PALABRAS_WORDLE_10K")
# return bbdd1k, bbdd10k

# def idiomas():
idiomaActual = "Español"
listaIdiomas = ["Español", "Català", "English"]
idiomaIndex = 0
# return idiomaActual, listaIdiomas, idiomaIndex


def cambiarIdioma():
    """ Funcion para cambiar el idioma por un botón
    """
    if entry_f1_c1.get() == "":
        global idiomaIndex, idiomaActual, bbdd1k, bbdd10k, palabraElejida, palabraElejida1, palabraElejida2, palabraElejida3
        # idiomaActual, listaIdiomas, idiomaIndex = idiomas()
        ...
        if idiomaActual == "Español":
            ...
            bbdd1k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PARAULES_WORDLE_1K")
            bbdd10k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PARAULES_WORDLE_10K")
            palabraElejida = palabraElejida2
        elif idiomaActual == "English":
            buttonIdioma.configure(text = "EN")
            bbdd1k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\WORDS_WORDLE_1K")
            bbdd10k = (r"C:\Users\MARTI\Desktop\PYTHON\BBDD\WORDS_WORDLE_10K")
            palabraElejida = palabraElejida3


def elegir_palabra_aleatoria():
    """ Funcion para elegir una palabra random de los tres idiomas cada día
    """
    ...
    miConexion2 = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PARAULES_WORDLE_1K")
    ...
    miConexion3 = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\WORDS_WORDLE_1K")
...
def gameWin():
    """ Funcion de la interfáz ganadora
    """
    ...
    labelWin = CTkLabel(master=app, text="", font=("Dubai", 33, "bold"), justify=CENTER)
    if idiomaActual == "Español":
        labelWin.configure(text="HAS GANADO")
    elif idiomaActual == "Català":
        labelWin.configure(text="HAS GUANYAT")
    elif idiomaActual == "English":
        labelWin.configure(text="YOU WIN")

    labelWin.place(relx=0.5, rely=0.1, anchor=CENTER)

    estadisticas()


def gameOver():
    """ Funcion de la interfáz perdedora
    """
    global listatodoslosentry
    for entry in listatodoslosentry:
        entry.configure(fg_color="#2b2b2b", bg_color="#2b2b2b", border_color="#2b2b2b", text_color="#2b2b2b")
    ...
    if idiomaActual == "Español":
        ...
        labelNoHasAcertado = CTkLabel(master=app, text="GAME OVER", font=("Dubai", 33, "bold"), justify=CENTER)
        global labelLaPalabraEra
        labelLaPalabraEra = CTkLabel(master=app, text="LA PALABRA ERA:", font=("Dubai", 22, "bold"), justify=CENTER)
    elif idiomaActual == "Català":
        labelNoHasAcertado = CTkLabel(master=app, text="GAME OVER", font=("Dubai", 33, "bold"), justify=CENTER)
        labelLaPalabraEra = CTkLabel(master=app, text="LA PARAULA ERA:", font=("Dubai", 22, "bold"), justify=CENTER)
    elif idiomaActual == "English":
        labelNoHasAcertado = CTkLabel(master=app, text="GAME OVER", font=("Dubai", 33, "bold"), justify=CENTER)
        labelLaPalabraEra = CTkLabel(master=app, text="THE WORD WAS:", font=("Dubai", 22, "bold"), justify=CENTER)
...
def estadisticas():
    ...
    """
    labelEstadisticas = CTkLabel(master=app, text="", font=("Dubai", 20, "bold"), justify=CENTER)
    labelPorcentaje = CTkLabel(master=app, text="", font=("Dubai", 11, "bold"), justify=CENTER)
    labelPlays = CTkLabel(master=app, text="", font=("Dubai", 11, "bold"), justify = CENTER)

    if idiomaActual == "Español":
        miConexion = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PALABRAS_WORDLE_1K")
        labelEstadisticas.configure(text="ESTADÍSTICAS")
        labelPorcentaje.configure(text="Porcentaje de acierto")
        labelPlays.configure(text="Jugadas") 
    elif idiomaActual == "Català":
        miConexion = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\PARAULES_WORDLE_1K")
        labelEstadisticas.configure(text="ESTADÍSTIQUES")
        labelPorcentaje.configure(text="Percentatge d'encert")
        labelPlays.configure(text="Jugades")
    elif idiomaActual == "English":
        miConexion = connect(r"C:\Users\MARTI\Desktop\PYTHON\BBDD\WORDS_WORDLE_1K")
        labelEstadisticas.configure(text="STATISTICS")
        labelPorcentaje.configure(text="Success rate")
        labelPlays.configure(text="Plays")

    miCursor = miConexion.cursor()
    wins = miCursor.execute("SELECT WINS FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    defeats = miCursor.execute("SELECT DEFEATS FROM ESTADISTICAS WHERE id = 1").fetchone()[0]

    wins_f1 = miCursor.execute("SELECT FILA1 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    wins_f2 = miCursor.execute("SELECT FILA2 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    wins_f3 = miCursor.execute("SELECT FILA3 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    wins_f4 = miCursor.execute("SELECT FILA4 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    wins_f5 = miCursor.execute("SELECT FILA5 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
    wins_f6 = miCursor.execute("SELECT FILA6 FROM ESTADISTICAS WHERE id = 1").fetchone()[0]
      
    plays = wins + defeats
    porcentaje = round((wins/plays)*100)
    porcentajeWins = str(porcentaje) + " %"

    wins_f1_p = round((wins_f1/plays)*100)
    wins_f2_p = round((wins_f2/plays)*100)
    wins_f3_p = round((wins_f3/plays)*100)
    wins_f4_p = round((wins_f4/plays)*100)
    wins_f5_p = round((wins_f5/plays)*100)
    wins_f6_p = round((wins_f6/plays)*100)
    # defeats_p = round((defeats/plays)*100)

    wins_f1_p = str(wins_f1_p) + " %"
    wins_f2_p = str(wins_f2_p) + " %"
    wins_f3_p = str(wins_f3_p) + " %"
    wins_f4_p = str(wins_f4_p) + " %"
    wins_f5_p = str(wins_f5_p) + " %"
    wins_f6_p = str(wins_f6_p) + " %"


    labelPorcentajeWins = CTkLabel(master=app, text=porcentajeWins, font=("Dubai", 33, "bold"), justify=CENTER)
    labelNPlays = CTkLabel(master=app, text=plays, font=("Dubai", 33, "bold"), justify=CENTER)

    labelWinFila1 = CTkLabel(master=app, text="1: "+str(wins_f1)+" ("+wins_f1_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    labelWinFila2 = CTkLabel(master=app, text="2: "+str(wins_f2)+" ("+wins_f2_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    labelWinFila3 = CTkLabel(master=app, text="3: "+str(wins_f3)+" ("+wins_f3_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    labelWinFila4 = CTkLabel(master=app, text="4: "+str(wins_f4)+" ("+wins_f4_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    labelWinFila5 = CTkLabel(master=app, text="5: "+str(wins_f5)+" ("+wins_f5_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    labelWinFila6 = CTkLabel(master=app, text="6: "+str(wins_f6)+" ("+wins_f6_p+")", font=("Dubai", 11, "bold"), justify=CENTER)
    # labelX = CTkLabel(master=app, text="X:"+defeats+"("+defeats_p+")", font=("Dubai", 11, "bold"), justify=CENTER)

    labelEstadisticas.place(relx=0.5, rely=0.3, anchor = CENTER)
    labelPorcentaje.place(relx=0.7, rely=0.4, anchor = CENTER)
    labelPorcentajeWins.place(relx=0.7, rely=0.47, anchor = CENTER)
    labelPlays.place(relx=0.25, rely=0.4, anchor = CENTER)
    labelNPlays.place(relx=0.25, rely=0.47, anchor = CENTER)

    labelWinFila1.place(relx=0.5, rely=0.55, anchor = CENTER)
    labelWinFila2.place(relx=0.5, rely=0.6, anchor = CENTER)
    labelWinFila3.place(relx=0.5, rely=0.65, anchor = CENTER)
    labelWinFila4.place(relx=0.5, rely=0.7, anchor = CENTER)
    labelWinFila5.place(relx=0.5, rely=0.75, anchor = CENTER)
    labelWinFila6.place(relx=0.5, rely=0.8, anchor = CENTER)


def actualizar_contador():
    """ Funcion para el contador de cuánto queda para la siguiente palabra (00:00)
    """
...
# root.resizable(0, 0)
# root.iconbitmap(r"C:\Users\MARTI\Desktop\PYTHON\INTERFACES GRÁFICAS\python_xuVsziQBJR.ico")
