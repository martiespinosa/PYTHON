...
def enviarCorreo(email): # script para enviar correo de confirmacion
    ...
    if not correo_registrado:
        ...
        if nombreEntry.get() != "" and apellidoEntry.get() != "" and email != "" and contraseñaEntry.get() != "":
            ...
            app = CTkFrame(raiz)
            app.pack(fill = "both", expand=True)
            mensaje = CTkLabel(master = app, text="Comprueba tu correo electrónico\ny confirma tu registro.\nEscribe el código que te hemos enviado", font=("Dubai", 18, "bold"))
            mensaje.place(relx=0.5, rely=0.15, anchor = CENTER)
            solo_numeros =  lambda text: text.isdecimal()
            maximo_seis = StringVar()
            codigoEntry = CTkEntry(master = app, placeholder_text="CÓDIGO", font=("Dubai", 15, "bold"), justify=CENTER, width=70, border_width=2, corner_radius=5, validate="key", validatecommand=(app.register(solo_numeros), "%S"), textvariable=maximo_seis)
            codigoEntry.place(relx=0.5, rely=0.35,  anchor = CENTER)
            enviarCodigoBoton = CTkButton(master = app, text="OK", font=("Dubai", 15, "bold"), width=70, height=25, anchor="center", cursor="hand2", command=lambda: confirmarRegistro(codigoEntry.get(), str(codigo)))
            enviarCodigoBoton.place(relx=0.5, rely=0.7, anchor = CENTER)
            ...
            cuerpo = "Hola, este es el código para confirmar tu registro:\n\n"+str(codigo)
...
contraseñaEntry = CTkEntry(master = raiz, placeholder_text="CONTRASEÑA", font=("Dubai", 11, "bold"), width=250, height=25, border_width=2, corner_radius=5)
contraseñaEntry.place(relx=0.5, rely=0.55, anchor = CENTER)
